...
public class Utility {
	...
	public static func computeBufferSize(
		formatPointer: UnsafePointer<AudioStreamBasicDescription>, queue: AudioQueueRef,
		seconds: Float64
	) -> UInt32 {
		var bytes: UInt32 = 0
		...
		if format.mBytesPerFrame > 0 {
			...
			if format.mBytesPerPacket == 0 {
				...
				AudioQueueGetProperty(
					queue, kAudioConverterPropertyMaximumOutputPacketSize, maxPacketSize,
					propertyDataSize)
			}

			var packets = format.mFramesPerPacket > 0 ? frames / format.mFramesPerPacket : frames
